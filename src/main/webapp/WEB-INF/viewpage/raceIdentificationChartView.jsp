            ...
            <div class="section">
               <div class="page-title">
                  <div class="title-details">
                     <h4>View Race Identification Chart</h4>
                     ...
                     <div class="card">
                        <div class="card-header">
                           <ul class="nav nav-tabs nav-fill" role="tablist">
                               <a class="nav-item nav-link "  href="raceIdentificationChart">Add</a>
                              <a class="nav-item nav-link active"  href="raceIdentificationChartView" >View</a>
                           </ul>
						...
						<div style="float: right;" id="pageId">
                            ...
                            <!-- Edit -->
                           <form action="raceIdentificationChartEdit" method="post" id="editForm">
                           	<input type="hidden" name="csrf_token_" value="<c:out value='${csrf_token_}'/>"/>
                           	<input type="hidden" id="stagId" name="raceChartId">
                           </form>
                        </div>
                        <!--===================================================-->
                     </div>
...
function callData(pSize,pageNumber)
{
	...
	$.ajax({
			type:"GET",
			url:"viewAllraceIdentificationChartPage",
			data:{
					"pageSize":pageSize,
			...
			success:function(response){
				...
				for(var i=array0.startPage;i<=array0.endPage;i++)
				{
				 ...
				 if(val!=" " || val!=null )
				{
					 listDataLen=listDataLen+val.raceChartList.length-backUp;
  					 var dataLen=val.raceChartList.length;
					 var indexLen=listDataLen-dataLen;
					
					$.each(val.raceChartList,function(index,value)
					{
									count=indexLen+index+1 + (10*array0.pageNo);
									 $("#p4").text(count);
									html=html+"<tr><td>"+count+"</td>";
									html=html+"<td>"+value.seq_1+"</td>";
									html=html+"<td>"+value.seq_2+"</td>";
									html=html+"<td>"+value.seq_3+"</td>";
									html=html+"<td>"+value.seq_4+"</td>"; 
...
function search(pSize,pageNumber){
	...
	$.ajax({
			type:"GET",
			url:"search-raceIdentificationChart",
			data:{
					"pageSize":pageSize,
					"pageNumber":pageNumber,
					"name":name,
