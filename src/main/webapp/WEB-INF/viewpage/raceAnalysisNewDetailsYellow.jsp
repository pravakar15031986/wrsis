	...
	// Submit
		 ...
		 $(document).on('click', '.submit', function(e) {
			...
			var	repeat = cls.split("_")[1];
			 ...
			 for(i=0;i<rowCount;i++)
				 {
				 	...
				 	var json = {"Remark":remark_,"InoculationId":$(inoculationId).html(),"AnalysisId":$(analysisId).html(),"WheatLineId":$(wheatlineId).html(),"HL":$(gname).val(),"InfectionType":$(fname+" option:selected").text()};
				 	jsa.push(json);
				 }
			 ...
			 $('#dataTable'+repeat+'  tr').each(function(index, tr) { 
				 if(index >1)
					 {
					 	var val_ = $(this).find('td:last').find('.result').text();
					 	arr.push(val_);
...
'			      <p class="class'+(j+1)+'_'+k+'_val result" style="font-weight:bold"></p><span style="display:none;" id="repeatation_analysis_inoculation_'+k+'_'+counter+'">'+inoculationId+'</span>'+
	...
	console.log(html_);
	/* alert();
	if($("table.table-bordered").length)
	{
	 */	
