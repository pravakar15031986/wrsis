...
// Search panel
$('.search-sec').addClass('open');
$('.searchopen i').toggleClass('icon-up-arrow');
$('.searchopen').on('click',function(){
$('.search-sec').slideToggle('open');
$(this).find('i').toggleClass('icon-up-arrow')
})
// Search panel
...
// Slim scroll funtion
  (function($){
    /*$(".notifications ul, .dashboard-portlet__content__details, .fixed-height").mCustomScrollbar({
	  ...
	  $(" .dashboard-portlet__content__details, .fixed-height").mCustomScrollbar({
	      theme:"inset-2-dark"
	    });
  })(jQuery);
  function loadNavigation(pgName, pgId, plinkId, titelBar, strFirstLink, strLastLink) {
	   ...
	   //alert(absPath);
		  console.log(pgName+' -- '+pgId+' -- '+plinkId+' -- '+path+' -- '+absPath);
		if (pgName == absPath) {
			//alert('0');
			$('.' + pgId).addClass('show').find('.dropdown-menu').addClass('show');
