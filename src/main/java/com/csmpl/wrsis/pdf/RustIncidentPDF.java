...
public class RustIncidentPDF {

	public static final Logger LOG = LoggerFactory.getLogger(RustIncidentPDF.class);
		...
		try {
			...
			for (int i = 0; i < beans.size(); i++) {
				...
				try {
					...
					LOG.error("RustIncidentPDF::generateRustIncidentPdf():" + e.getMessage());
