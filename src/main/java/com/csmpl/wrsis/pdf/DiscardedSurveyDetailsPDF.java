...
public class DiscardedSurveyDetailsPDF {

	public static final Logger LOG = LoggerFactory.getLogger(DiscardedSurveyDetailsPDF.class);

	public ByteArrayInputStream generateDiscardedSurveyPdf(List<SurveyDetailsBean> beans) {

		ByteArrayOutputStream out = new ByteArrayOutputStream();
		String[] cells = { "Sl. No.", "Survey No", "Survey Date", "Discarded Date", WrsisPortalConstant.REGION, WrsisPortalConstant.ZONE, WrsisPortalConstant.WOREDA,
				WrsisPortalConstant.KEBELE, "Institution Name", "Type of Rust", "Mode of Data Collection", "Reason" };
		...
		try {
			...
			Paragraph head = new Paragraph("Discarded Survey",
			...
			for (int i = 0; i < beans.size(); i++) {
				...
				cell1 = getPdfPCellOject(sdb.getRemark());
				table.addCell(cell1);
			}
			...
			LOG.error("DiscardedSurveyDetailsPDF::generateDiscardedSurveyPdf():" + e.getMessage());
		}
