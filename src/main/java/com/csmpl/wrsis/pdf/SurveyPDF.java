...
public class SurveyPDF {

	public static final Logger LOG = LoggerFactory.getLogger(SurveyPDF.class);

	public ByteArrayInputStream generateSurveyDetailsPdf(List<SurveyDetailsBean> beans) {
		ByteArrayOutputStream out = new ByteArrayOutputStream();
		String[] cells = { "Sl. No.", "Survey No", "Survey Date", "Wheat Type", WrsisPortalConstant.REGION, "Location Details",
				"Rust Type", "Collect Mode" };
		...
		try {
			...
			Paragraph head = new Paragraph("Survey Details",
			...
			for (int i = 0; i < beans.size(); i++) {
				...
				cell1 = getPdfPCellOject(sdb.getWheatType());
				table.addCell(cell1);

				cell1 = getPdfPCellOject(sdb.getRegion());
				table.addCell(cell1);

				cell1 = getPdfPCellOject(sdb.getLocation());
				table.addCell(cell1);
			...
			LOG.error("SurveyPDF::generateSurveyDetailsPdf():" + e.getMessage());
		}
